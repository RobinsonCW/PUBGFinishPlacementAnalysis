...
A complete set of pair plots and box plots are available in the EDA appendix. 

A number of variables are not useful for prediciton. Id, groupID, and matchId are identifiers. MatchType, DBNOs, and Revives are not relevant because we are focusing on the Solo match type. matchDuration, maxPlace, and numGroups are static across any given match, even if they vary individually, so there is no information about individual player performance. Finally, our response variable, top.10, is derived from winPlacePerc, so we cannot use it for prediction.  
...
```{r rf.model.appendix, eval=FALSE}
...
```{r rf.model.plot.appendix, eval=FALSE}
...
```{r rf.variable.importance.appendix, eval=FALSE}
...
```{r rf.confusion.matrix.appendix, eval=FALSE}
confusionMatrix(data=prd.rf.test,
                reference=test.rf$top.10, "Yes")
```
...
```{r rf.area.under.curve.appendix, eval=FALSE}
plotRoc <- function(preds, truth) {
  pred <- prediction(preds, truth)
  roc.perf = performance(pred, measure = "tpr", x.measure = "fpr")
  auc.train <- performance(pred, measure = "auc")
